...
function inlineCSS(fileName: string, dir: string): PluginOption {
  return {
    name: 'so-inline-css-vite-plugin',
    apply: 'build',
    closeBundle() {
...
export function getESMBundleConfig(
  soConfig: Record<string, any>
): InlineConfig {
  // SoUI(so-ui.esm.js)
  // soConfig 应该只用到了name 那先定个常量代替
  // const name = get(soConfig, 'name')
  const name = 'SoUi'
...
export function getUMDConfig(soConfig: Record<string, any>): InlineConfig {
  // SoUI(so-ui.js)
  // soConfig 应该只用到了name 那先定个常量代替
  // const name = get(soConfig, 'name')
  const name = 'SoUi'
...
// export function getBuildConfig(soConfig: Record<string, any>): InlineConfig {
