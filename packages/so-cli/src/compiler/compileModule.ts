...
export function compileUMD() {
  return new Promise<void>((resolve, reject) => {
    const config = getUMDConfig(getSoConfig())
    build(config)
      .then(() => resolve())
      .catch(err => {
        console.log(err, 'errcompileUmd')
        reject()
      })
  })
}
...
export function compileESMBundle() {
  return new Promise<void>((resolve, reject) => {
    const config = getESMBundleConfig(getSoConfig())
    build(config)
      .then(() => resolve())
      .catch(err => {
...
export async function compileModule(
  modules: 'umd' | 'commonjs' | 'esm-bundle' | 'module'
) {
  if (modules == 'umd') {
    // 打包成umd格式
    await compileUMD()
